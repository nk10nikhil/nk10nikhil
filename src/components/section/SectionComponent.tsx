...
const SectionComponent = ({ section, isActive, onClick }: SectionComponentProps) => {
    ...
    useEffect(() => {
        if (isActive && sectionRef.current) {
            // Scroll the section into view when it becomes active, but only if not already visible
            if (!isVisible) {
    ...
    return (
        <div
            ref={sectionRef}
            id={section.id}
            className={`py-8 transition-all duration-500 ease-in-out cursor-pointer border-b border-gray-100 dark:border-gray-800
        ...
        >
            ...
            <div className="section-service-tags mt-4">
                {section.serviceTags.map((tag) => (
                    <button key={tag.id} className="service-tag">
                        {tag.name}
                    </button>
                ))}
            </div>
